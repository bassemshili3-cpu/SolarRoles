...
export default async function LinemanJobsPage({ searchParams }: any) {
  const params = await searchParams
    ...
    getMergedJobCount({ what: params.what || 'lineman', where: params.where || '' }),
    searchMergedJobs({ what: params.what || 'lineman', where: params.where || '', results_per_page: 30, salary_min: params.salary_min ? Number(params.salary_min) : undefined }),
  ])
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <header className="mb-10">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">
            {count > 0 ? count.toLocaleString('en-US') : ''} Lineman Jobs Available Across the United States
          </h1>
        </header>
