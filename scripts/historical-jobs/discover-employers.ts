...
function parseOptions(args: string[]): CliOptions {
  ...
  return {
    ...
    reservoirSamplesPerSource: intArg(args, '--reservoir-samples-per-source', 32),
...
function buildBatchSql(
  ...
  const jobDetailPredicate = `(
    ...
    OR (ends_with(host, '.applytojob.com') AND regexp_matches(url_path, '^/apply/[A-Za-z0-9]{8,}/[^/]+/?$'))
    OR (ends_with(host, '.breezy.hr') AND regexp_matches(lower(url_path), '^/p/[a-z0-9-]{8,}/[^/]+/?$'))
...
async function main() {
  ...
  console.log(`[employer-discovery] ${selectedCrawls.join(', ')} | ${parquetFiles.length} Parquet files | ${batches.length} batch(es) | OPEN employer discovery v4 | ${options.threads} threads | ${options.memoryLimit}`)
