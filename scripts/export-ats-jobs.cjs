...
async function main() {
  ...
  const jobs = await prisma.job.findMany({
    ...(allSources ? {} : { where: { source: { in: exportedSources } } }),
    orderBy: [{ source: 'asc' }, { id: 'asc' }],
  });
  if (!jobs.length) throw new Error(`No ${allSources ? '' : 'ATS '}jobs matched; no export written.`);
  ...
  const scope = allSources ? 'all-jobs' : includeAdzuna ? 'ats-plus-adzuna' : 'ats-only';
  const file = path.join(directory, `solarroles-${scope}-${timestamp}.csv`);
  fs.writeFileSync(file, csv, { flag: 'wx' });
  const sourceNames = allSources ? sources.map(({ source }) => source) : exportedSources;
  console.log(JSON.stringify({ file, count: jobs.length, columns: columns.length, active: jobs.filter(job => job.active).length, bySource: Object.fromEntries(sourceNames.map(source => [source, jobs.filter(job => job.source === source).length])), bytes: fs.statSync(file).size }));
}
