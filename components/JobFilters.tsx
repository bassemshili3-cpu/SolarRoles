  ...
  return (
    ...
    <div className="sticky top-20 space-y-3 md:space-y-6 bg-card p-3 md:p-6 rounded-2xl border shadow-lg">
      ...
      <div className="space-y-2 md:space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 md:py-3 bg-background border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-sm"
          />
        </div>
            ...
            >
              {filteredSuggestions.map((loc, index) => (
                <button
                  ...
                  className="w-full px-4 py-2 md:py-3 text-left text-sm flex items-center gap-3 transition-colors border-b border-gray-100 last:border-b-0 hover:bg-blue-50"
                  style={{ color: '#111827' }}
                >
        ...
        <Button onClick={applyFilters} className="w-full py-2 md:py-3 text-sm md:text-base font-semibold">
      ...
      <div className="space-y-2 md:space-y-4">
        ...
        <div className="py-2 md:py-4 px-2 bg-gradient-to-r from-primary/5 to-primary/10 rounded-xl">
        ...
        <div className="flex items-center justify-center bg-primary/10 px-4 py-2 md:py-3 rounded-lg border border-primary/20">
          <div className="text-center">
            <div className="text-xs text-muted-foreground uppercase tracking-wide">Minimum</div>
            <div className="text-xl md:text-2xl font-medium text-primary">${salary.toLocaleString()}</div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 md:gap-2">
          {[50000, 75000, 100000, 125000, 150000].map((preset) => (
            <button
              ...
              className="text-xs px-2 md:px-3 py-1 rounded-full bg-secondary text-secondary-foreground hover:bg-secondary/80 transition-colors"
            >
              ${preset / 1000}k
            </button>
          ))}
      ...
      <div className="space-y-2 md:space-y-3">
        ...
        <div className="grid grid-cols-2 gap-1.5 md:gap-2">
          {['Full-time', 'Part-time', 'Contract', 'Remote'].map((type) => (
            <div
              key={type}
              className={`flex items-center gap-2 p-2 md:p-3 rounded-lg border cursor-pointer transition-all ${
