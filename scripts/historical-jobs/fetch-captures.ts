...
async function main() {
  ...
  console.log(`[fetch] ${selected.length} captures | concurrency=${concurrency}`)
  await Promise.all(Array.from({ length: Math.min(concurrency, selected.length) }, () => worker()))

  const completedResults = results.filter((row): row is Record<string, unknown> => Boolean(row))
  await writeFile(path.join(root, 'fetch-results.jsonl'), completedResults.map((row) => `${JSON.stringify(row)}\n`).join(''))
