      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        ...
        <header className="mb-12">
          <div className="flex items-center gap-3 mb-3">
            <DollarSign className="w-6 h-6 text-[#C9991F]" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          <div className="border border-[#EDE5F0] rounded-xl p-5 text-center">
            <p className="text-2xl font-bold text-[#3D1654]">${fmt(nationalAvg)}</p>
            <p className="text-xs text-gray-500 mt-1">National avg.</p>
          </div>
          <div className="border border-[#EDE5F0] rounded-xl p-5 text-center">
            <p className="text-2xl font-bold text-[#3D1654]">{fmt(totalListings)}</p>
            <p className="text-xs text-gray-500 mt-1">Active listings</p>
          </div>
          <div className="border border-[#EDE5F0] rounded-xl p-5 text-center">
            <p className="text-2xl font-bold text-[#C9991F]">{topState ? `$${fmt(topState.avgSalary)}` : '—'}</p>
        ...
        {/* ── STATE TABLE ── */}
        <section>
          <h2 className="text-xl font-bold text-[#3D1654] mb-4">
            {jobTitle} Average Salary by State
          </h2>
          <div className="border border-[#EDE5F0] rounded-2xl overflow-hidden">
           
            <div className="grid grid-cols-4 gap-px bg-[#EDE5F0] text-xs font-medium text-gray-500 uppercase tracking-wider">
            ...
            {salaryByState.map((row, i) => {
              ...
              return (
                <div
                  ...
                  className={`grid grid-cols-4 gap-px ${i % 2 === 0 ? 'bg-[#EDE5F0]' : 'bg-[#EDE5F0]'}`}
                >
                  <div className={`${i % 2 === 0 ? 'bg-white' : 'bg-[#FAF7FC]'} px-4 py-3 text-sm font-bold text-gray-400`}>
                    {i + 1}
                  </div>
                  <div className={`${i % 2 === 0 ? 'bg-white' : 'bg-[#FAF7FC]'} px-4 py-3 text-sm`}>
                    <Link
                      href={`/data/states/${row.addressRegion.toLowerCase().replace(/\s+/g, '-')}`}
                      className="text-gray-800 hover:text-[#C9991F]"
                    >
                      {row.addressRegion}
                    </Link>
                  </div>
                  <div className={`${i % 2 === 0 ? 'bg-white' : 'bg-[#FAF7FC]'} px-4 py-3 text-sm text-right`}>
                    <span className="font-semibold text-[#3D1654]">${fmt(row.avgSalary)}</span>
                    <span className={`ml-2 text-xs ${diff >= 0 ? 'text-[#C9991F]' : 'text-gray-400'}`}>
                      {diff >= 0 ? '+' : ''}{diff.toFixed(0)}%
                    </span>
                  </div>
                  <div className={`${i % 2 === 0 ? 'bg-white' : 'bg-[#FAF7FC]'} px-4 py-3 text-sm text-right text-gray-500`}>
                    {row.count}
                  </div>
                </div>
              )
        ...
        <section className="mt-16 bg-[#F5EEF7] border border-[#E8D5F0] rounded-2xl p-8 text-center">
          <h2 className="text-xl font-bold text-[#3D1654] mb-2">Browse {jobTitle} Jobs</h2>
