    ...
    <div className="overflow-x-auto rounded-xl border border-gray-200">
      <table className="w-full min-w-[580px] text-left text-sm">
        ...
        <tbody className="divide-y divide-gray-200">{filtered.map(row => <tr key={row.id} className="align-top">
          <th scope="row" className="w-[38%] p-4 font-normal"><span className="block font-bold">{row.company}</span><a href={row.jobUrl} rel="nofollow" className="mt-1 block text-[#744600] underline underline-offset-2">{row.title}</a><span className="mt-2 block text-xs text-gray-500">{row.listingCount} listing{row.listingCount > 1 ? 's' : ''} grouped</span></th>
          <td className="p-4"><p className="font-semibold">{row.travelLabel}</p><details className="mt-3"><summary className="cursor-pointer text-[#744600] underline underline-offset-2">Read the evidence</summary>
            <div className="mt-3 space-y-3 text-sm leading-6 text-gray-700"><p><strong>Remote signal ({row.remoteSource === 'description' ? 'description excerpt' : 'title / location'}):</strong> {row.remoteEvidence}</p>
              ...
              {row.notes && <p>{row.notes}</p>}<a href={row.sourceUrl} rel="nofollow" className="inline-block text-[#744600] underline">Source listing</a>
            </div>
          </details></td>
        </tr>)}</tbody>
      </table>
